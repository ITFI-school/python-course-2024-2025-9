...
class Asteroid(Wrapper):
    ...
    def die(self):
        ...
        # если астероидов и вражеских кораблей не осталось, переходим на следующий уровень
        if AlienShip.total == 0 and Asteroid.total == 0:
            self.game.advance()
...
class AlienShip(Wrapper):
    """ Спрайт Вражеский корабль, движущийся по экрану. """
    SMALL = 1
    MEDIUM = 2
    LARGE = 3
    images = { SMALL  : games.load_image("alien_ship_small.bmp"),
               MEDIUM : games.load_image("alien_ship_med.bmp"),
               LARGE  : games.load_image("alien_ship_big.bmp") }
    SPEED = 1.5
    SPAWN = 3
    POINTS = 50
    total =  0
    ...
    def __init__(self, game, pos_x, pos_y, size):
        ...
        super().__init__(
            image = AlienShip.images[size],
            x = pos_x, y = pos_y,
            dx = random.choice([1, -1]) * AlienShip.SPEED * random.random() / size, 
            dy = random.choice([1, -1]) * AlienShip.SPEED * random.random() / size)
    ...
    def die(self):
        """ Разрушает вражеский космический корабль. """
        AlienShip.total -= 1

        self.game.score.value += int(AlienShip.POINTS / self.size)
        self.game.score.right = games.screen.width - 10   
        
        # если размеры вражеского корабля крупные или средние, заменить
        # его двумя более мелкими астероидами 
        if self.size != AlienShip.SMALL:
            for i in range(AlienShip.SPAWN):
                new_elient_ship = AlienShip(game = self.game,
                                        pos_x = self.x,
                                        pos_y = self.y,
                                        size = self.size - 1)
                games.screen.add(new_elient_ship)
...
class Ship(Collider):
    ...
    def update(self):
        ...
        def get_speed_delta(is_forward):
            # рассчитывает изменение горизонтальной и вертикальной 
            # скорости корабля с учетом его угла поворота
            radian_angle = self.angle * math.pi / 180  # преобразование в радианы
            delta_x = Ship.VELOCITY_STEP * math.sin(radian_angle)
            delta_y = Ship.VELOCITY_STEP * -math.cos(radian_angle)
        ...
        if games.keyboard.is_pressed(games.K_UP):
            Ship.sound.play()
            self.dx, self.dy = get_speed_delta(is_forward=True)

        # корабль тормозит по нажатию стрелки вниз
        if games.keyboard.is_pressed(games.K_DOWN):
            Ship.sound.play()
            self.dx, self.dy = get_speed_delta(is_forward=False)

        # корабль останавливается по нажатию кнопки End
        if games.keyboard.is_pressed(games.K_END):
            self.dx, self.dy = 0, 0
...
class Game:
    ...
    def advance(self):
        ...
        # сохранить его от врагов сразу же после создания
        BUFFER = 150
     
        # создание новых астероидов и вражеских кораблей
        for i in range(self.level):
     
            for j in range(2):   # создаём и астероид, и вражеский космический корабль
                ...
                alien_x = self.ship.x + x_distance
                alien_y = self.ship.y + y_distance

                # если необходимо, вернем объект внутрь окна
                alien_x %= games.screen.width
                alien_y %= games.screen.height
           
                if j:
                    # создадим астероид
                    new_alien = Asteroid(game = self,
                                            pos_x = alien_x, pos_y = alien_y,
                                            size = Asteroid.LARGE)
                else:
                    # создадим корабль пришельцев
                    new_alien = AlienShip(game = self,
                                          pos_x = alien_x, pos_y = alien_y,
                                          size = AlienShip.LARGE)
